<?php
    ...
    $fields = array(
            ...
            createField("LearnerInfo.Identification.Photo"),
    ...
    if (isset($_GET['t']) && $_GET['t'] > 0){
        sendJSON($xml, "ECV", $fields);
    } else {
        $xml->save("europass.xml");
        echo toFancyHTML($xml->saveXML());
        echo toFancyHTML(json_encode($fields));
        echo toFancyHTML(json_encode(simplexml_load_string($xml->saveXML())));
    }*/
    ...
    function getInfos(){
        $arr = array();
        $arr["FirstName"] = "Antonio Oliveira da";
        $arr["Surname"] = "Silva";
