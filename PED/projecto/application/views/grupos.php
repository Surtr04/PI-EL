...
*/
	...
	if ($perms['I']) echo '<p>'.TPL::LinkAdicionar('groups/insere', 'Inserir novo grupo').'</p><br/><br/>';
	

	echo '<table style="margin:10px;"><tr><th class="th_normal">Grupo</th>'.(($perms['U'] || $perms['D']) ? '<th class="th_normal">Acções</th>' : '').'</tr>';
	...
	echo '</div>';
	function isApagavel($perms, $id){ return $perms['D'] && $id != Kohana::$config->load('defs.admin') && $id != Kohana::$config->load('defs.visitante'); }
?>